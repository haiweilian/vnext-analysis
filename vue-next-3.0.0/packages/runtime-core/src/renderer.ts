 ...
 * ```
 */
// VUENEXT-初始化流程 3-创建渲染器(createRenderer => baseCreateRenderer)
...
// implementation
// VUENEXT-初始化流程 3.1-创建渲染器(createRenderer => baseCreateRenderer)
...
function baseCreateRenderer(
  ...
  // VUENEXT-初始化流程 8.1-执行创建或者更新组件(patch)
  const patch: PatchFn = (
    ...
    switch (type) {
      ...
      default:
        if (shapeFlag & ShapeFlags.ELEMENT /* 1 */) {
          // VUENEXT-初始化流程 13-处理普通元素节点(processElement)
          // VUENEXT-更新流程 6-处理普通元素节点(processComponent)
          processElement(
            n1,
            n2,
            container,
        ...
        } else if (shapeFlag & ShapeFlags.COMPONENT /* 6 */) {
          // VUENEXT-初始化流程 9-处理组件(processComponent)
          // VUENEXT-更新流程 5-处理组件(processComponent)
          processComponent(
            n1,
            n2,
            container,
        ...
        } else if (shapeFlag & ShapeFlags.TELEPORT /* 64 */) {
          // 处理 TELEPORT
          ...
          shapeFlag & ShapeFlags.SUSPENSE /* 128 */
        ) {
          // 处理 SUSPENSE
  ...
  const processElement = (
    ...
    if (n1 == null) {
      // VUENEXT-初始化流程 13.1-挂载节点(mountElement)
      mountElement(
        n2,
        container,
        anchor,
      ...
      // VUENEXT-更新流程 6.1-更新普通元素节点(patchElement)
  ...
  const mountElement = (
    ...
    if (
      ...
      // VUENEXT-初始化流程 13.2-创建 DOM 元素节点(hostCreateElement)
      ...
      if (shapeFlag & ShapeFlags.TEXT_CHILDREN) {
        // VUENEXT-初始化流程 13.3-子节点是纯文本(hostSetElementText)
        // hostSetElementText 调用的是第 [2.1] 步合并的渲染配置，请看 `nodeOps` 文件中的 [13.3.1] 步。
        hostSetElementText(el, vnode.children as string)
      } else if (shapeFlag & ShapeFlags.ARRAY_CHILDREN) {
        // VUENEXT-初始化流程 13.4-子节点是数组(mountChildren)
        // 循环处理子节点
        mountChildren(
      ...
      // VUENEXT-初始化流程 13.5-设置处理 Props(hostPatchProp)
    ...
    // VUENEXT-初始化流程 13.6-把创建的元素节点挂载到容器上(hostInsert)
  ...
  const patchElement = (
    ...
    // VUENEXT-更新流程 6.2-更新props(patchProps)
    if (patchFlag > 0) {
    ...
    if (dynamicChildren) {
      ...
      // full diff
      // VUENEXT-更新流程 6.2-更新子节点(patchChildren)
      patchChildren(
        n1,
        n2,
        el,
  ...
  const processComponent = (
    ...
    if (n1 == null) {
      if (n2.shapeFlag & ShapeFlags.COMPONENT_KEPT_ALIVE) {
        ...
        // VUENEXT-初始化流程 9.1-挂载组件(mountComponent)
        mountComponent(
          n2,
          container,
          anchor,
      ...
      // VUENEXT-更新流程 5.1-更新子组件(updateComponent)
  ...
  const mountComponent: MountComponentFn = (
    ...
    // VUENEXT-初始化流程 12-设置并运行带副作用的渲染函数(setupRenderEffect)
    setupRenderEffect(
      instance,
      initialVNode,
      container,
  ...
  const updateComponent = (n1: VNode, n2: VNode, optimized: boolean) => {
    ...
    // VUENEXT-更新流程 5.2-根据新旧子组件 vnode 判断是否需要更新子组件(shouldUpdateComponent)
    ...
    if (shouldUpdateComponent(n1, n2, optimized)) {
      if (
        ...
        // normal update
        // 如果需要更新
        // 新的子组件 vnode 赋值给 instance.next。如果存在则证明是主动执行子组件的 update 方法。对照 setupRenderEffect 方法的 next 判断。
        instance.next = n2
        ...
        // 执行子组件的副作用渲染函数
        // VUENEXT-更新流程 5.3-执行子组件的副作用渲染函数，回到第 [1] 步的流程
        instance.update()
      }
    } else {
  ...
  const setupRenderEffect: SetupRenderEffectFn = (
    ...
    instance.update = effect(function componentEffect() {
      if (!instance.isMounted) {
        ...
        // 得出：如果是父组件主动执行子组件的重新渲染方法时，在渲染之前需要更新组件的 props、slots。
        if (next) {
        ...
        // VUENEXT-更新流程 3-渲染组件生成新的子树 vnode (renderComponentRoot)
        ...
        // VUENEXT-更新流程 4-根据新旧子树 vnode 做 patch
        patch(
          prevTree,
          nextTree,
  ...
  const patchKeyedChildren = (
    ...
    // 3. 前后节点 diff 完之后。旧的没有，新的有多余的节点 挂载剩余的新节点，
    // (a b)
    // (a b) c
    // i = 2, e1 = 1, e2 = 2
    // (a b)
    ...
    // 4. 前后节点 diff 完之后。新的没有，旧的有多余的节点 删除剩余的旧节点，
    // (a b) c
    // (a b)
    // i = 2, e1 = 2, e2 = 1
    // a (b c)
    ...
    // i = 2, e1 = 4, e2 = 5
      ...
      console.log('建立新子序列的索引图', keyToNewIndexMap)
  ...
  // VUENEXT-初始化流程 7.1-执行渲染器渲染VNode(render(vnode, container))
  // 组件渲染的核心逻辑
  const render: RootRenderFunction = (vnode, container) => {
    if (vnode == null) {
      ...
      // VUENEXT-初始化流程 8-调用创建或者更新组件(patch)
  ...
  return {
    render,
    hydrate,
    // VUENEXT-初始化流程 4-创建实例API(createAppAPI)
